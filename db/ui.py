...
engine = create_engine('postgresql+psycopg2://zaur:wsad1221@localhost:5432/zaurdb2')
...
with column_2:
    ...
    if dataset is not None:
        ...
        if st.button('Save dataset to database'):
            write_dataset('%s' % (dataset_name), dataset, engine)
            st.info('**%s** saved to database' % (dataset_name))

    try:
        read_title = st.empty()
        dataset_to_read = st.selectbox('Please select dataset to read', ([x[0] for x in list_datasets(engine)]))
        read_title.header('Read datasets')
        if st.button('Read dataset'):
            df = read_dataset(dataset_to_read, engine)
            st.subheader('Chart')

...
def save_company_to_db(ticker):
    ...
    conn_string = get_connection_string('postgres', 'psycopg2', 'zaur', passwd, 'localhost', '5432', 'zaurdb')
    ...
    if not Company.objects.filter(symbol=symbol, exchange=overview['Exchange']).exists():
        c = Company.objects.create(symbol=symbol, exchange=overview['Exchange'])

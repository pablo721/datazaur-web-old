...
def sectors_performance():
    key = '1WEDCC91HCX5F7U4'
    df = web.get_sector_performance_av(api_key=key)
    df.to_sql('markets_sectors_performance', engine, if_exists='replace', index=True)


def load_indices():
    df = investpy.indices.get_indices()
    df.to_sql('markets_indices', engine, if_exists='replace', index=False)
...
def load_indices_values():
    ...
    for country in investpy.get_index_countries():
        try:
            ...
            df.to_sql('markets_indices2', engine, if_exists='append', index=False)
...
def load_fx_rates():
    fx = investpy.get_currency_crosses_overview('USD', n_results=300)
    fx.to_sql('markets_fx_rates', engine, if_exists='replace', index=False)



def load_commodities():
    groups = ['metals', 'softs', 'grains', 'meats', 'energy']
    engine.execute('truncate table markets_commodity2;')
    for group in groups:
        df = investpy.get_commodities(group)
        if group == 'metals':
            df = df.loc[:10]
        df.to_sql(f'markets_commodity2', engine, if_exists='append', index=False)


def wb_regions():
    regions = pd.DataFrame(json.loads(requests.get('http://api.worldbank.org/v2/region?format=json').text)[1]).iloc[:, 1:]
    regions.to_sql('macro_region', engine, if_exists='replace', index=False)


def wb_topics():
    topics = pd.DataFrame(json.loads(requests.get('http://api.worldbank.org/v2/topic?format=json').text)[1])
    topics.to_sql('macro_wbtopic', engine, if_exists='replace', index=False)


def wb_incomelevels():
    levels = pd.DataFrame(json.loads(requests.get('http://api.worldbank.org/v2/incomelevel?format=json').text)[1])
    levels.to_sql('macro_wbincomelevel', engine, if_exists='replace', index=False)


def wb_lendingtypes():
    types = pd.DataFrame(json.loads(requests.get('http://api.worldbank.org/v2/lendingtypes?format=json').text)[1])
    types.to_sql('macro_wblendingtype', engine, if_exists='replace', index=False)
...
def wb_countries():
    ...
    for i, row in countries.iterrows():
        if not Country.objects.filter(id=row['id']).exists():
...
def wb_sources():
    ...
    df.to_sql('macro_source', engine, if_exists='replace', index=False)


def wb_indicators():
    indicators = wb.get_indicators()
    indicators.drop(labels=['sourceOrganization', 'unit'], axis=1, inplace=True)
    indicators.to_sql('macro_indicator', engine, if_exists='replace', index=False)
